...
import {
  getProductsByPurpose,
  PURPOSE_LABELS,
  PURPOSES,
  type PurposeMission,
} from "@/lib/content";
import { PURPOSE_DESCRIPTIONS } from "@/lib/product-classifications";
import { getPurposeIcon } from "@/components/CategoryIcons";
import ProductGrid from "@/components/ProductGrid";

export function generateStaticParams() {
  return PURPOSES.map((mission) => ({ mission }));
}
...
export default async function PurposePage({
  ...
  const products = getProductsByPurpose(mission as PurposeMission);
  const description = PURPOSE_DESCRIPTIONS[mission as PurposeMission];
  const IconComp = getPurposeIcon(mission);
